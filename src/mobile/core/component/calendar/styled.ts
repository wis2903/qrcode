...
export const StyledCalendarComponentHeaderNav = styled.button<{
...
export const StyledCalendarComponentBodyDates = styled(FlexboxComponent)<{ $readOnly?: boolean }>`
    ${StyledCalendarComponentBodyDateItem} {
...
export const StyledCalendarComponentContainer = styled.div<{
    $noBorder?: boolean;
    $readOnly?: boolean;
}>`
    border: 1px solid;
    border-radius: ${token.get<string>('component.calendar.radius')};
    padding: ${token.get<string>('component.calendar.padding')};
    background-color: ${(props): string =>
        props.$readOnly
            ? token.get<string>('global.color.grey-9')
            : token.get<string>('component.calendar.background-color')};
