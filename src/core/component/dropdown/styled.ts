...
const dropdownComponentItemStyle = (props: IStyledDropdownComponentItemProps): string => `
    ...
    color: ${props.$disabled ? token.get<string>('global.color.grey-4') : props.$color || 'unset'};
    background-color: ${
        props.$disabled
...
export const StyledDropdownComponentMenu = styled(ScrollAreaComponent)`
    ...
    border-radius: ${token.get<string>('component.dropdown.menu.radius')};
...
export const StyledDropdownComponentContent = styled.div<{
    ...
    ${(props): string => {
        ...
        return `
            color: white;
            position: relative;
            padding: ${token.get<string>('global.space.xxxs')} ${token.get<string>(
                'global.space.xxs'
            )};
            background: ${token.get<string>('global.color.navi-1')};
            ...
            ${((): string => {
                switch (props.$horizontalDirection) {
                    case HorizontalDirectionEnum.left:
                        return `
                            margin-left: ${props.$marginLeft || '-8px'};
                        `;
                    case HorizontalDirectionEnum.right:
                        return `
                            margin-right: ${props.$marginRight || '-8px'};
                        `;
                    default:
                        return '';
                }
