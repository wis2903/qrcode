...
const columnContent = (props: ITableColumnProps): string => {
    return `
        ...
        ${((): string => {
            if (!props.$isInChildRow && !props.$isInSelectedRow) return '';

            if (props.$isInSelectedRow) {
                return `
                    background-color: ${token.get<string>('global.color.grey-9')} !important;
                `;
            }

            return `
                background-color: ${token.get<string>('global.color.grey-9')} !important;
                border-bottom-color: ${token.get<string>('global.color.grey-6')} !important;
            `;
        })()}
...
export const StyledTableComponentContainer = styled(FlexboxComponent)<{
    ...
    tbody {
        ...
        ${(props): string => {
            if (props.$enabledBorderBottomForLastRow) {
                return `
                ${StyledTableComponentCell} {
                    border-bottom: 1px solid ${token.get<string>(
                        'component.table.body.divider-color'
                    )};
                }   
                `;
            }
