...
export class PandaDate {
    ...
    static plus(days: number): PandaDate {
        const dt = new Date();
        dt.setDate(dt.getDate() + days);
        return new PandaDate(dt);
    }
    ...
    toStringWithTimezone({ timezoneInt, lang }: { timezoneInt: number; lang?: LangEnum }): string {
        const dt = new Date(
            `${this._instance.getUTCFullYear()}/${
                this._instance.getUTCMonth() + 1
            }/${this._instance.getUTCDate()} ${this._instance.getUTCHours()}:${this._instance.getUTCMinutes()}:${this._instance.getUTCSeconds()}`
        );
        dt.setHours(dt.getHours() + timezoneInt);
        return new PandaDate(dt).toString(lang);
    }
    ...
    toDateTimeStringWithTimezone({
        ...
        dt.setHours(dt.getHours() + timezoneInt);
        return new PandaDate(dt).toDateTimeString(lang);
    }

    toDateTimeStringForReport(lang?: LangEnum): string {
        const locale = getCalendarLocale(lang);
        const d = this._instance.getDate();
        const m = this._instance.getMonth();
        const y = this._instance.getFullYear();
